...
router.post("/register-compitition/:activityId", requireLoginUser, async (req, res) => {
  ...
  try {
    ...
    // 3️⃣ Allow if either main club is artclub OR joinedClubs contains artclub
    if (user.club !== "artclub" && !user.joinedClubs.includes("artclub")) {
      return res.status(403).json({ message: "You must be in the art club before registering for this competition" });
    }
...
router.post("/unregister-compitition/:activityId", requireLoginUser, async (req, res) => {
  const userId = req.user._id;
  const { activityId } = req.params;
...
router.post("/upload-photo-compitition/:eventId", requireLoginUser, async (req, res) => {
  try {
    const { pic } = req.body;
...
router.get("/has-uploaded-compitition/:eventId", requireLoginUser, async (req, res) => {
  const userId = req.user._id.toString();
  const eventId = req.params.eventId;
